define([
	...
	var $content = $('#content');
	var control, $element, $select1, $select2, $select3;
	...
	QUnit.module("CategorySelectControl", {
		setup: function(){
			...
			control = new CategorySelectControl($element);
			$select1 = $element.find('.select1');
			$select2 = $element.find('.select2');
			$select3 = $element.find('.select3');
		},
		teardown: function(){
			$element.remove()
	...
	QUnit.asyncTest("selecting before update", function () {
	...
	QUnit.asyncTest("selecting", function () {
		...
		control.update().done(function() {
			...
			console.log($select1.find('option'));
