...
docker run -d -e "SYMFONY_ENV=docker" -v "$(cd ..;pwd)/data:/data" --link "elasticsearch:elasticsearch" --link "mysql:mysql" --link "redis:redis" -p 8080:80 --name apache-php apache-php
